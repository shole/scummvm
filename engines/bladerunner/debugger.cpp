...
bool Debugger::cmdOverlay(int argc, const char **argv) {
	...
	//Graphics::Surface *tempsurf = new Graphics::Surface(640,480,640,640*480);

	//_vm->_overlays->removeAll();

	Common::String mixlist[12] = {
		"MODE.MIX", // 0
		"MUSIC.MIX", // 1
		"OUTTAKE1.MIX", // 2
		"OUTTAKE2.MIX", // 3
	...
	//for (int mixindex = 3; mixindex < 12; mixindex++) {
		...
		for (int i = 0; i < 318; i++) {
			...
			if (!_vqaPlayer->open()) {
				...
				debugPrintf("%d / %d frames", _vqaPlayer->_frame, _vqaPlayer->getFrameCount());
				if (_vqaPlayer->_frame <= 0) {
					for (int i = 0; i < _vqaPlayer->getFrameCount(); i++) {
						_vqaPlayer->update(true, true, false);
						//_vqaPlayer->update();
					}
					debugPrintf("done!");
				}
	...
	//surface.create(640, 480, gameDataPixelFormat());
